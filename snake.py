        ...
        for j in range(1, 13):
            if [j, i] not in S:
                if p == 0:
                    ...
                    p -= 1
def init():
    F = [['⚪️' for i in range(14)] for j in range(14)]
    for i in range(14):
        F[0][i] = '⚫️'
        F[i][0] = '⚫️'
        F[13][13 - i] = '⚫️'
        F[13 - i][13] = '⚫️'
    h = [random.randint(2, 11), random.randint(2, 11)]
    d = 'l'
    m = h[0] - 1
    S = [h]
    if 14 - h[0] > m:
...
def step(F, S, d, h, p, score):
    if 1:
        ...
        x, y = h
        if h in S[4:]:
            return (False, F, S, d, h, p, score)
        if x == 0 or x == 13 or y == 0 or y == 13:
            return (False, F, S, d, h, p, score)
        return (True, F, S, d, h, p, score)
